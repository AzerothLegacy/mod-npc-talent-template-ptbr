...
public:
    ...
    bool OnGossipHello(Player* player, Creature* creature)
    {
        ...
        {
            AddGossipItemFor(player, GOSSIP_ICON_INTERACT_1, "|cff00ff00|TInterface\\icons\\Trade_Engineering:30:30|t|r Redefinir talentos", GOSSIP_SENDER_MAIN, GOSSIP_ACTION_RESET_TALENTS, "Tem certeza de que deseja redefinir seus talentos?", 0, false);
            if (player->getClass() == CLASS_HUNTER)
                AddGossipItemFor(player, GOSSIP_ICON_INTERACT_1, "|cff00ff00|TInterface\\icons\\ability_hunter_beasttaming:30:30|t|r Redefinir talentos do pet", GOSSIP_SENDER_MAIN, GOSSIP_ACTION_RESET_PET_TALENTS, "Tem certeza de que deseja redefinir os talentos do seu pet?", 0, false);
        }

        if (sTemplateNpcMgr->enableRemoveAllGlyphs)
            AddGossipItemFor(player, GOSSIP_ICON_INTERACT_1, "|cff00ff00|TInterface\\icons\\Spell_ChargeNegative:30|t|r Remover todos os glyphs", GOSSIP_SENDER_MAIN, GOSSIP_ACTION_RESET_REMOVE_GLYPHS, "Tem certeza de que deseja remover todos os seus glyphs?", 0, false);

        if (sTemplateNpcMgr->enableDestroyEquippedGear)
            AddGossipItemFor(player, GOSSIP_ICON_INTERACT_1, "|cff00ff00|TInterface\\icons\\ability_vehicle_launchplayer:30|t|r Deletar itens equipados", GOSSIP_SENDER_MAIN, GOSSIP_ACTION_RESET_REMOVE_EQUIPPED_GEAR, "Tem certeza de que deseja deletar todos os seus itens equipado?", 0, false);
